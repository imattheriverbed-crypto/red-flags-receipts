      ...
      <section id="signup" className="relative min-h-[70vh] bg-primary flex items-center justify-center px-6 py-20 transition-colors duration-700">
        <div className="text-center max-w-2xl">
          ...
          <p className="font-mono-flag text-sm uppercase tracking-[0.2em] text-ink/70 mb-2">
            The warning arrives Tuesday, August 4th at 9PM.
          </p>
    ...
    >
      ...
      <div className="w-full max-w-3xl text-center">
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto">
          ...
          {/* Optional engagement fields — encouraged brand fields */}
          <div className="mt-6 text-left">
            <button
              type="button"
              onClick={() => setShowExtras((v) => !v)}
              className="inline-flex items-center gap-2 font-mono-flag text-[11px] uppercase tracking-[0.2em] text-ink/70 hover:text-ink transition-colors"
            >
              <ChevronDown className={`w-3.5 h-3.5 transition-transform ${showExtras ? 'rotate-180' : ''}`} />
              Tell us your red flags {traits.length > 0 || note.trim() ? `(${[traits.length > 0 && `${traits.length} traits`, note.trim() && 'note'].filter(Boolean).join(', ')})` : '(optional)'}
            </button>

            {showExtras && (
              <div className="mt-4 bg-ink/5 border border-ink/15 p-5 text-left">
                <p className="font-mono-flag text-[10px] uppercase tracking-[0.2em] text-ink/60 mb-3">
                  Which drops are you here for? <span className="text-ink/40 normal-case tracking-normal">(pick any)</span>
                </p>
                  ...
                  {TRAITS.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        className={`font-mono-flag text-[10px] uppercase tracking-[0.15em] px-3 py-2 border transition-colors ${
                          on ? 'bg-primary text-parchment border-primary' : 'bg-transparent text-ink/70 border-ink/30 hover:border-primary hover:text-primary'
                        }`}
                      >
                        {on && <Check className="inline w-3 h-3 mr-1" />}{t}
                      </button>
                    );
                  })}
                </div>

                <p className="font-mono-flag text-[10px] uppercase tracking-[0.2em] text-ink/60 mb-2">
                  Leave a red flag note <span className="text-ink/40 normal-case tracking-normal">(the one you survived)</span>
                </p>
                <textarea
                  value={note}
                  ...
                  rows={3}
                  maxLength={280}
                  placeholder="He said he'd change..."
                  className="w-full px-4 py-3 font-body text-sm bg-parchment text-ink border border-ink/30 placeholder-ink/30 focus:outline-none focus:border-primary resize-none"
                />
                <p className="font-mono-flag text-[9px] uppercase tracking-[0.15em] text-ink/35 mt-1 text-right">
                  {note.length}/280
                </p>
              </div>
            )}
        ...
        <p className={`font-mono-flag text-[10px] uppercase tracking-[0.2em] mt-6 ${activated ? 'text-ink/50' : 'text-ink/40'}`}>
          Early access · Launch discount · Tuesday, August 04 · 9PM
        </p>
      </div>
    </section>
  );
