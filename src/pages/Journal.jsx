...
import DesignInspirations from '@/components/journal/DesignInspirations';
import ConfessionWall from '@/components/journal/ConfessionWall';

const TABS = [
  { key: 'field-notes', label: 'Field Notes' },
  { key: 'inspirations', label: 'Design Inspirations' },
  { key: 'confess', label: 'Confess' },
];

export default function Journal() {
  const [tab, setTab] = useState('field-notes');
    ...
    <div className="dark bg-ink text-parchment min-h-screen">
      ...
      <header className="relative pt-40 pb-12 px-6 sm:px-12 text-center overflow-hidden grain-overlay">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          ...
          <p className="font-body text-sm uppercase tracking-[0.25em] text-parchment/50 mb-10">
            The stories behind every warning
          </p>

          {/* Tabs */}
          <div className="inline-flex flex-wrap justify-center border border-primary/25">
            {TABS.map((t) => (
              <button
                key={t.key}
                onClick={() => setTab(t.key)}
                className={`px-5 sm:px-7 py-3 font-mono-flag text-[11px] uppercase tracking-[0.22em] transition-colors ${tab === t.key ? 'bg-primary text-parchment' : 'text-parchment/70 hover:text-primary'}`}
              >
                {t.label}
              </button>
            ))}
